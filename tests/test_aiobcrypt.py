...
async def test_gensalt_basic(monkeypatch):
    monkeypatch.setattr(os, "urandom", lambda n: b"0000000000000000")
    assert await aiobcrypt.gensalt() == b"$2b$12$KB.uKB.uKB.uKB.uKB.uK."
...
async def test_gensalt_rounds_valid(rounds, expected, monkeypatch):
    monkeypatch.setattr(os, "urandom", lambda n: b"0000000000000000")
    assert await aiobcrypt.gensalt(rounds) == expected


@pytest.mark.parametrize("rounds", list(range(1, 4)))
async def test_gensalt_rounds_invalid(rounds):
    with pytest.raises(ValueError):
        await aiobcrypt.gensalt(rounds)


async def test_gensalt_bad_prefix():
    with pytest.raises(ValueError):
        await aiobcrypt.gensalt(prefix="bad")
...
async def test_hashpw_new(password, salt, hashed):
    assert await aiobcrypt.hashpw(password, salt) == hashed


@pytest.mark.parametrize(("password", "salt", "hashed"), _test_vectors)
async def test_checkpw(password, salt, hashed):
    assert await aiobcrypt.checkpw(password, hashed) is True


@pytest.mark.parametrize(("password", "salt", "hashed"), _test_vectors)
async def test_hashpw_existing(password, salt, hashed):
    assert await aiobcrypt.hashpw(password, hashed) == hashed


@pytest.mark.parametrize(("password", "hashed", "expected"), _2y_test_vectors)
async def test_hashpw_2y_prefix(password, hashed, expected):
    assert await aiobcrypt.hashpw(password, hashed) == expected


@pytest.mark.parametrize(("password", "hashed", "expected"), _2y_test_vectors)
async def test_checkpw_2y_prefix(password, hashed, expected):
    assert await aiobcrypt.checkpw(password, hashed) is True


async def test_hashpw_invalid():
    with pytest.raises(ValueError):
        await aiobcrypt.hashpw(b"password", b"$2z$04$cVWp4XaNU8a4v1uMRum2SO")


async def test_checkpw_wrong_password():
    assert (
        await aiobcrypt.checkpw(
            b"badpass",
            b"$2b$04$2Siw3Nv3Q/gTOIPetAyPr.GNj3aO0lb1E5E9UumYGKjP9BYqlNWJe",
        )
        is False
...
async def test_checkpw_bad_salt():
    with pytest.raises(ValueError):
        await aiobcrypt.checkpw(
            b"badpass",
...
async def test_checkpw_str_password():
    with pytest.raises(TypeError):
        await aiobcrypt.checkpw("password", b"$2b$04$cVWp4XaNU8a4v1uMRum2SO")


async def test_checkpw_str_salt():
    with pytest.raises(TypeError):
        await aiobcrypt.checkpw(b"password", "$2b$04$cVWp4XaNU8a4v1uMRum2SO")


async def test_hashpw_str_password():
    with pytest.raises(TypeError):
        await aiobcrypt.hashpw("password", b"$2b$04$cVWp4XaNU8a4v1uMRum2SO")


async def test_hashpw_str_salt():
    with pytest.raises(TypeError):
        await aiobcrypt.hashpw(b"password", "$2b$04$cVWp4XaNU8a4v1uMRum2SO")


async def test_checkpw_nul_byte():
    await aiobcrypt.checkpw(
        b"abc\0def",
        b"$2b$04$2Siw3Nv3Q/gTOIPetAyPr.GNj3aO0lb1E5E9UumYGKjP9BYqlNWJe",
    )
...
async def test_hashpw_nul_byte():
    salt = await aiobcrypt.gensalt(4)
    hashed = await aiobcrypt.hashpw(b"abc\0def", salt)
    assert await aiobcrypt.checkpw(b"abc\0def", hashed)
    # assert that we are sensitive to changes in the password after the first
    # null byte:
    assert not await aiobcrypt.checkpw(b"abc\0deg", hashed)
    assert not await aiobcrypt.checkpw(b"abc\0def\0", hashed)
    assert not await aiobcrypt.checkpw(b"abc\0def\0\0", hashed)


async def test_checkpw_extra_data():
    salt = await aiobcrypt.gensalt(4)
    hashed = await aiobcrypt.hashpw(b"abc", salt)

    assert await aiobcrypt.checkpw(b"abc", hashed)
    assert await aiobcrypt.checkpw(b"abc", hashed + b"extra") is False
    assert await aiobcrypt.checkpw(b"abc", hashed[:-10]) is False
...
async def test_kdf(rounds, password, salt, expected):
    derived = await aiobcrypt.kdf(
...
async def test_kdf_str_salt():
    with pytest.raises(TypeError):
        await aiobcrypt.kdf(b"password", "salt", 10, 10)


async def test_kdf_no_warn_rounds():
    await aiobcrypt.kdf(b"password", b"salt", 10, 10, True)


async def test_kdf_warn_rounds():
    with pytest.warns(UserWarning):
        await aiobcrypt.kdf(b"password", b"salt", 10, 10)
...
async def test_invalid_params(password, salt, desired_key_bytes, rounds, error):
    with pytest.raises(error):
        await aiobcrypt.kdf(password, salt, desired_key_bytes, rounds)


async def test_2a_wraparound_bug():
    assert (
        await aiobcrypt.hashpw(
